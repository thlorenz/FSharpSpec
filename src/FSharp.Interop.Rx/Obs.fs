...
module Obs =
  ...
  let createObserver next error completed =
    let onNext = next |> action1
    let onError = error |> actionExn
    let onCompleted = completed |> action
  ...
  let concat (observable : IObservable<'a>) = Observable.Concat(observable)
  ...
  /// Extensions below added by me (T.L.)

  /// Projects each value of an observable sequence into a buffer that's sent out when either it's full 
  /// or a given amount of time has elapsed.
  let bufferWithTimeOrCount timeSpan length scheduler  observable = 
    Observable.BufferWithTimeOrCount(observable, timeSpan, length, scheduler)
